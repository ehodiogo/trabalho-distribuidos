...
router.get("/", async (req, res) => {
  ...
  try {
    const result = await readPool.query("SELECT * FROM clients LIMIT 100");
    console.log("[GET] /clients → Retornando", result.rows.length, "clientes");

    const countResult = await readPool.query("SELECT COUNT(*) FROM clients");
    const totalClients = parseInt(countResult.rows[0].count, 10);

    console.log("clientes de um total de", totalClients);
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const result = await readPool.query("SELECT * FROM clients WHERE id = $1", [
      id,
    ]);
